...
class MPCController:
    ...
    def _build_mpc(self):
        ...
        # 新版 do-mpc 推荐通过 settings 配置；set_param 仍然兼容，但这里使用新版写法。
        ...
        # do-mpc 的 set_rterm 是对 Δu 的软惩罚；硬限幅在 compute_control 中继续保留。
...
class BalloonPlatformEnv(gym.Env):
    def __init__(self):
        ...
        self.u_mpc = 0.0
        self.step_count = 0
        self.max_steps = int(300 / self.Ts)
    ...
    def reset(self, seed=None, options=None):
        ...
        obs = np.concatenate([self.state.flatten(), [noise], [self.u_mpc]]).astype(np.float32)
        return obs, {}
    ...
    def step(self, action):
        ...
        x_ref = np.array([[theta_ref], [0], [0], [0]], dtype=float)

        # MPC 控制（RL 环境）：SAC 输出作为残差控制叠加到 MPC 上。
        u_mpc = self.mpc_controller.compute_control(self.state, x_ref, t)
        u_sac = float(action[0]) * 5.0
        u_total = float(np.clip(u_mpc + u_sac, -self.u_max, self.u_max))

        # 更新 RL 状态
        u_input = np.array([[u_total]], dtype=float)
        x_dot = self.A @ self.state + self.B @ u_input
        state_rl = self.state + self.Ts * x_dot
        state_rl[0] = np.clip(state_rl[0], -np.pi, np.pi)
        state_rl[1] += noise
        y_rl = float((self.C @ state_rl).item())

        # 纯 MPC 状态更新
        u_mpc_pure = self.mpc_controller_pure.compute_control(self.state_mpc, x_ref, t)
        u_input_mpc = np.array([[u_mpc_pure]], dtype=float)
        ...
        y_mpc = float((self.C @ self.state_mpc).item())

        # 奖励（保持原样，只把总控制输入换成 MPC+SAC 的 u_total）
        angle_error = float(np.abs(y_rl - theta_ref))
        reward = float(-0.1 * angle_error - 0.01 * u_total ** 2 + np.exp(-100 * angle_error))
        ...
        info = {
            ...
            "u_mpc": u_mpc,
            "u_sac": u_sac,
            # 兼容旧代码字段名：如果你还有旧脚本引用这些 key，不会直接报错。
            "angle_lqr": y_mpc,
            "u_lqr": u_mpc,
            "u_rl": u_sac,
        }
